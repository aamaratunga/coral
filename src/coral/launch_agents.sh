...
TARGET_DIR="${1:-.}"
AGENT_TYPE="${2:-claude}"
LAUNCH_AGENTS="${3:-}"  # Pass "agents" as 3rd arg to auto-launch agents
LOG_DIR="${TMPDIR:-/tmp}"
...
# Clean up all existing coral logs
rm -f "${LOG_DIR}/"*_coral_*.log
...
if [ "$LAUNCH_AGENTS" = "agents" ]; then
    ...
    echo "=== $AGENT_TYPE Coral Launcher (Independent Sessions) ==="
    ...
    for dir in "${worktrees[@]}"; do
        ...
        log_file="${LOG_DIR}/${AGENT_TYPE}_coral_${session_id}.log"
...
"
    ...
    echo "=== Coral Dashboard ==="
    echo "Starting web server only (launch agents from the dashboard)."
    echo "To also launch agents: launch-coral <path> <agent-type> agents"
    echo ""
fi
